class User < ApplicationRecord
  ...
  def createUserAPI(params)
    ...
    response = Oj.load(curlCall)
    if !response.blank? && response['success']
      self.update(serviceFee: 35, uuid: response['uuid'],username: response['username'], accessPin: response['accessPin'], phone: response['phone'], twilioPhoneVerify: response['twilioPhoneVerify'] )
      return response
    else
      return response
    end
