class User < ApplicationRecord
  ...
  def createStripeCardTokenAPI(params)
    ...
    curlCall = `curl -H "appName: #{ENV['appName']}" -H "bxxkxmxppAuthtoken: #{self.authentication_token}" -d "number=#{number}&exp_month=#{exp_month}&exp_year=#{exp_year}&cvc=#{cvc}" #{SITEurl}/api/v1/stripe-tokens`
